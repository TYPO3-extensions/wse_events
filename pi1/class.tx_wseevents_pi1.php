...
//			$fullTable.=$this->pi_list_browseresults();
	...
	function old_listView($content,$conf)	{
		...
		if ($this->piVars['showUid'])	{	// If a single element should be displayed:
			$this->internal['currentTable'] = 'tx_wseevents_events';
			$this->internal['currentRow'] = $this->pi_getRecord('tx_wseevents_events',$this->piVars['showUid']);
			...
			$this->internal['results_at_a_time']=t3lib_div::intInRange($lConf['results_at_a_time'],0,1000,3);		// Number of results to show in a listing.
			$this->internal['maxPages']=t3lib_div::intInRange($lConf['maxPages'],0,1000,2);;		// The maximum number of "pages" in the browse-box: "Page 1", "Page 2", etc.
			$this->internal['searchFieldList']='name';
			$this->internal['orderByList']='uid,name';
	
				// Get number of records:
			$res = $this->pi_exec_query('tx_wseevents_events',1);
			list($this->internal['res_count']) = $GLOBALS['TYPO3_DB']->sql_fetch_row($res);
	
				// Make listing query, pass query to SQL database:
			$res = $this->pi_exec_query('tx_wseevents_events');
			$this->internal['currentTable'] = 'tx_wseevents_events';
